...
| Status | Item | Notes |
|--------|------|-------|
| ✅ | **Environment validation on startup** | Startup `log.warn` when mainnet Binance live is configured without confirm; `CONFIRMED_LIVE` accepted as alias for `CONFIRMED_LIVE_TRADING` (`config.ts`); `create-runtime.ts` still throws if unset — `index.ts` + `config.ts` |
